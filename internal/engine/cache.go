...
// v198: Ruby reads db/structure.sql when present — the database's own account of the
...
// guessed.
// v199: Stimulus markup bindings become named facts. A `data-controller="x"` or
...
// v200: finding 0007's next markup slice, fail-closed at every shape. Literal Turbo
...
// v201: importmap-rails apps are detected as JavaScript projects. The TS extractor
...
// v202: the rest of the view-composition surface, fail-closed at every shape. A
...
// v203: CommonJS export assignments declare symbols. `exports.name = function` and
...
// v204: Go interface methods declare symbols. An interface declaration's named
...
// v205: three census-named vocabulary gaps closed. The TS extractor claims and
...
//
// v206: TypeScript class members carry their decorators, and get accessors become
...
// one.
//
// v207: a Rails namespace's declared table_name_prefix corrects the models nested
...
// v208: the Rails default schema dump joins the SQL one. db/schema.rb is read
...
//
// v209: a JavaScript class says which base class it extends, and which module
...
// same import table.
// v210: NOT an extractor change — the first entry here that is not. Owner resolution
...
// pinned by a v209 build and graded by this one reports 30 moved findings over an
...
// v211: a TypeScript constructor is a fact. The member walk skipped `#`-private
...
// header states rather than on cache staleness: a baseline pinned by a v210 build
// must not grade a tree that now carries constructors as unchanged.
// v212: a TypeScript method records the fields it assigns on itself, as the Ruby
...
// v213: a TypeScript member records whether it declares a parameter at all. A
...
// the header states rather than on cache staleness: a baseline pinned by a v212
// build must not grade a tree whose members now carry this prop as unchanged.
// v214: takes_parameters reaches the module-level function symbols it always
// meant to cover. v213 emitted it only from the class-member walk, so the
