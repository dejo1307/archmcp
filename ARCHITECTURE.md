...
| `calls`        | A symbol calls another symbol resolved inside the indexed graph |
| `calls_unresolved` | An invocation whose target Enola could not identify |
| `calls_runtime` | A language runtime, standard-library, or prelude invocation |
| `calls_external` | An invocation resolved to an imported package outside the graph |
...
- **Hotspots** ([`internal/explainers/hotspots`](internal/explainers/hotspots/hotspots.go)) — flags call-graph **pinch points** (symbols with both high fan-in and unique resolved-call fan-out, scored `fanIn × fanOut`). Constructors, enum variants, type references, runtime/external calls and unresolved calls stay queryable but do not increase architectural centrality; repeated invocations count once for degree and retain their frequency separately. A cheap degree-centrality proxy for betweenness — chokepoints most call chains pass through.
...
- **Dead-methods** ([`internal/explainers/deadmethods`](internal/explainers/deadmethods/deadmethods.go)): reports Ruby methods whose bare name no resolved call, unresolved short-name call, or explicit name reference in the graph uses, and methods only spec files name. It does not follow calls: the name is looked up across the evidence the graph holds (symbol to symbol, template to method, spec to method, the callback, delegate, serializer and naming DSLs, symbol-to-proc block arguments, symbol arguments passed for later dispatch). An unresolved call is weak evidence that can keep a matching method alive without creating architectural fan-in, deliberately biasing toward a missed lead rather than a false accusation. Scoped to surfaces whose callers the graph can see: services, jobs, workers, queries and lib. Models, controllers, views, components, helpers, presenters, serializers, JSON:API resources, GraphQL types, Avo, mailers and one-off surfaces (tasks, seeds, importers) are ignored, as is any name matching a recorded dynamic-send prefix, a framework entry name, a writer, an operator or a Ruby protocol method. Emitted below `1.0`: a name-based index keeps a method alive whenever any same-named method is called, so the list under-reports and every entry is a candidate, never a verdict.
...
| `relation` | Filter by relation kind (`declares`, `imports`, `calls`, `calls_unresolved`, `calls_runtime`, `calls_external`, `implements`, `depends_on`). |
...
| `relation_kinds` | Restrict to `imports`, `calls`, `calls_unresolved`, `calls_runtime`, `calls_external`, `declares`, `implements`, `depends_on`, `has_method`. Default: all. |
